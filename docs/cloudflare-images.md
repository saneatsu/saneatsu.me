...
> **NOTE:** SNS向けのOGPメタデータでも `large` バリアントを使用します。幅1200pxが確保されるため、Twitter/Xのlarge card要件を満たす一方で、追加のバリアントを増やす必要がありません。現在は動的OG画像の背景として `large` バリアントを合成しているため、共通レイアウトとサムネイル背景を両立できます。
>
