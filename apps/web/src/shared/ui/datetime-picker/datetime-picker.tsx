...
function genMonths(
	...
	return Array.from({ length: 12 }, (_, i) => ({
		value: i,
		label: format(new Date(2021, i), "M月", { locale }),
	}));
}
		...
		<DayPicker
			...
			classNames={{
				...
				button_previous: cn(
					...
					"h-10 w-10 bg-transparent opacity-50 hover:opacity-100 absolute left-5 top-5 cursor-pointer",
					disableLeftNavigation() && "pointer-events-none"
				),
				button_next: cn(
					buttonVariants({ variant: "outline" }),
					"h-10 w-10 bg-transparent opacity-50 hover:opacity-100 absolute right-5 top-5 cursor-pointer",
