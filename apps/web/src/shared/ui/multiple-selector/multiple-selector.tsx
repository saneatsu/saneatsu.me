...
>(
	(
		...
		const debouncedSearchTerm = useDebounce(inputLabel, delay || 500);
		...
		const CreatableItem = () => {
			...
			if (
				isOptionsExist(options, [{ value: inputLabel, label: inputLabel }]) ||
				selected.find((s) => s.label === inputLabel)
			) {
				return undefined;
			}
			...
			if (!onSearch && inputLabel.length > 0) {
				return Item;
			}
		...
		return (
			...
			>
				<div
					...
					tabIndex={0}
					className={cn(
						"min-h-10 rounded-md border border-input text-sm ring-offset-background",
					...
					<div className="flex flex-wrap gap-1">
						...
						<CommandPrimitive.Input
							...
							onValueChange={(label) => {
								...
								inputProps?.onValueChange?.(label);
							}}
							onBlur={(event) => {
								setOpen(false);
