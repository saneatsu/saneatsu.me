...
- [x] H1のタイトルを一番上に書く
- [x] その下にタグを表示する
- [x] タグをクリックしたら /tags/:id に遷移すること
- [x] Markdownで記事を表示する
- [x] ステータスが公開済みの場合は表示されること
- [x] 下書き、アーカイブ済みの場合は404にすること
- [x] apps/backend, apps/web のテストを書くこと
- [x] 右サイドに見出しごとのアウトラインが表示されること。クリックしたらアンカーリンクで飛ぶ
...
- [x] article_translationsテーブルにカラムを追加するか検討
  - URLごと、つまり言語ごとにカウント
- [ ] 記事詳細ページにView数を表示する？
  - する、しないのメリデメから考えたい
- [x] 管理画面の記事一覧ページで記事の合計の閲覧数を表示する「閲覧数」というカラムを追加して
- [x] 管理画面の記事一覧ページで閲覧数でソートできるようにする
- [x] 管理画面のダッシュボードページで、直近30、90、180、360日の閲覧数の推移をグラフで見ることができる。 https://ui.shadcn.com/docs/components/chart を参考にして recharts を使ってください。与えた画面のようなイメージです。最終更新の下、つまり総記事数の上に表示してください。
- [x] seed.ts を更新して現在から360日前までの閲覧数推移を見れるようにしましょう。その後シードをローカルに適用させてください
- [x] 与えた画像のようにBar Chartを使ってください。ref: https://ui.shadcn.com/charts/bar#charts。必要であれば https://v0.dev/chat/open-in-v0-tMd9MccdGaK にコードがあります。


### Scrapboxのような仕様

記事内で他の記事や見出しへのリンクを簡単に作成できる Wiki Link 機能を実装する。

#### 対応するユースケース

1. **知識のネットワーク化**
   - 関連する技術記事を相互に参照し、読者の理解を深める
   - 例: Next.js記事から[[React基礎]]や[[TypeScript入門]]へリンク
   - 効果: 前提知識や関連知識への導線が明確になる

2. **シリーズ記事の連携**
   - 連載記事を順番に読み進められるナビゲーション
   - 例: [[TypeScript入門 Part 1]]→[[TypeScript入門 Part 2]]→[[TypeScript入門 Part 3]]
   - 効果: 読者が迷わずシリーズを完読できる

3. **用語集・辞書機能**
   - 専門用語の解説記事へワンクリックでアクセス
   - 例: "[[SSR]]とは〜"、"[[レンダリング]]の種類"
   - 効果: 初心者でも専門用語を理解しながら読み進められる

4. **トピッククラスターの形成**
   - 関連トピックをまとめて体系的に学習
   - 例: Webパフォーマンス記事から[[Core Web Vitals]]、[[画像最適化]]、[[キャッシュ戦略]]へ
   - 効果: 特定分野の知識を網羅的に習得

5. **見出しへのダイレクトリンク**
   - 長い記事の特定セクションへ直接ジャンプ
   - 例: [[Next.js完全ガイド#ルーティング]]、[[React最適化#メモ化]]
   - 効果: 必要な情報に素早くアクセス

6. **更新履歴・変更点の追跡**
   - バージョンアップ情報の差分を明確化
   - 例: [[Next.js 14新機能]]から[[Next.js 13との違い#破壊的変更]]へ
   - 効果: アップデート時の影響範囲を把握しやすい

7. **FAQ・トラブルシューティング**
   - よくある問題の解決策へ誘導
   - 例: "ビルドエラーは[[トラブルシューティング#ビルドエラー]]を参照"
   - 効果: 問題解決までの時間短縮

8. **多言語間での知識共有**
   - 日本語で書いた[[Next.jsの基本]]が英語では[[Next.js Basics]]として表示
   - 効果: 言語の壁を超えた知識の共有

#### 基本仕様
- [ ] **記法**: `[[記事タイトル]]` 形式でリンクを作成
- [ ] **内部保存**: slug形式で保存（例: `[[nextjs-basics]]`）
- [ ] **表示変換**: 閲覧言語に応じたタイトルに自動変換
- [ ] **見出しリンク**: `[[記事タイトル#見出し]]` で特定セクションへのリンク

#### サジェスト機能
- [ ] **トリガー**: `[[` 入力で自動的にポップアップ表示
- [ ] **検索対象**: 
  - [ ] 記事タイトル（全言語）
  - [ ] 記事内の見出し（#, ##, ### レベル）
- [ ] **表示内容**:
...
- [ ] **キー構造**:
  - [ ] `suggestions:ja` - 日本語の全サジェストデータ
  - [ ] `suggestions:en` - 英語の全サジェストデータ
  - [ ] `headings:[slug]:[lang]` - 個別記事の見出しキャッシュ
- [ ] **更新タイミング**:
  - [ ] 記事作成・更新時に自動更新
  - [ ] TTL: 24時間
- [ ] **パフォーマンス目標**:
...
- [ ] **GET /api/articles/suggestions**
  - [ ] クエリパラメータ: q（検索文字列）、lang（言語）、limit（最大件数）
  - [ ] レスポンス: サジェスト配列（slug、title、type、headingLevel）
- [ ] **キャッシュ層の実装**
  - [ ] Cloudflare KV統合
  - [ ] フォールバック処理（KVが空の場合）
...
#### レンダリング実装
- [ ] **リンク変換処理**
  - [ ] `[[slug]]` → `<a href="/[locale]/articles/slug">タイトル</a>`
  - [ ] `[[slug#heading]]` → アンカーリンク付き
- [ ] **存在チェック**
  - [ ] 存在する記事: 通常のリンクスタイル（青色、下線）
  - [ ] 存在しない記事: 赤色、点線で表示
- [ ] **記事表示コンポーネントへの統合**
  - [ ] ArticleDetailViewでの変換処理
  - [ ] サーバーサイドでのレンダリング対応
...
- `#xxx` の場合はタグとする
  - ObsidianやScrapboxと同じ

## GitHub Actions

/Users/saneatsu/Codes/Nito/nito を参考にして

- [ ] mainブランチにマージしたらCfのProduction環境にデプロイ
- [ ] PRではCfのPreview環境にデプロイ
- [ ] PRではBiomeのチェック、Vitestのチェックを行う

## Chores

- [ ] Ctrl + Kはカタカナに変換するときに使用しています。Markdown Editorで他のショートカットに割り当てないでください。Cmd + Lのショートカットも機能させないようにして
- [x] Tabで移動しているときにPopoverがスクロールしてほしい
- [x] EscでPopoverが閉じるようにしてほしい
- [ ] `locale === "ja"` のような書き方はせずに next-intl を使用する
- [x] apps/web でHono Clientを使わずにAPIを叩いている箇所は api/ ディレクトリにReact Queryを使ったHooksを定義してその中でHono Clientを使う型式にして
- [x] ヘッダー・フッターのpaddingが左側が0になっているのできれいにしたい
- [x] タイトルは `saneatsu.me` にする
- [x] ヘッダーの言語選択（LanguageSwitcher統合済み）
- [ ] 言語設定は nitoプロジェクトのヘッダーを参考にする（今はアイテムの右側にチェックマークがあるけど、左に丸が表示されていてほしい。つまりテーマ設定と同じUIにしたい）
...
- [ ] 月に1回リンク確認。ボタンをクリックしたら手動でも可能。全部のコンテンツをなめていって有効なものかチェックして
- [ ] タイトル、記事を含めたあいまい検索
  - 「技術 負債」の場合「技術負債」でも「技術に関する様々な負債」という文章でもヒットするようにする
  - また、ヒットした箇所の前後50文字を切り取って約100文字程度の文章を結果として表示してマーカーを引いておきたい
- [ ] トップページのdescriptionに記事は書きかけでも公開するので随時更新されることを明記しておく
